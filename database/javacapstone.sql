-- phpMyAdmin SQL Dump
-- version 5.2.1
-- https://www.phpmyadmin.net/
--
-- Host: 127.0.0.1
-- Generation Time: May 17, 2026 at 01:54 PM
-- Server version: 10.4.32-MariaDB
-- PHP Version: 8.2.12

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
START TRANSACTION;
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;

--
-- Database: `javacapstone`
--

-- --------------------------------------------------------

--
-- Table structure for table `patients`
--

CREATE TABLE `patients` (
  `patient_id` int(11) NOT NULL,
  `user_id` int(11) NOT NULL,
  `full_name` varchar(255) DEFAULT NULL,
  `mobile_number` varchar(20) DEFAULT NULL,
  `birthday` date DEFAULT NULL,
  `gender` varchar(10) DEFAULT NULL,
  `address` text DEFAULT NULL,
  `profile_picture` longblob DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `status` varchar(20) NOT NULL DEFAULT 'active'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `patients`
--

INSERT INTO `patients` (`patient_id`, `user_id`, `full_name`, `mobile_number`, `birthday`, `gender`, `address`, `profile_picture`, `created_at`, `status`) VALUES
(1, 1, 'Juner John Caimor', '912345678', '1910-10-07', 'Female', 'sa among house', , '2026-05-08 08:42:32', 'active'),
(2, 4, 'John Doe', '09123456789', NULL, NULL, NULL, NULL, '2026-05-08 09:20:31', 'inactive'),
(3, 5, 'Venz Virni T. Blanza', '09919106710', NULL, NULL, NULL, NULL, '2026-05-16 05:15:35', 'active');

-- --------------------------------------------------------

--
-- Table structure for table `queue_form`
--

CREATE TABLE `queue_form` (
  `form_id` int(11) NOT NULL,
  `user_id` int(11) DEFAULT NULL,
  `first_name` varchar(20) NOT NULL,
  `middle_initial` varchar(4) NOT NULL,
  `last_name` varchar(20) NOT NULL,
  `birth_date` date DEFAULT NULL,
  `age` int(11) NOT NULL,
  `gender` varchar(6) NOT NULL,
  `civil_status` varchar(50) NOT NULL,
  `patient_type` varchar(40) NOT NULL,
  `address` varchar(255) NOT NULL,
  `nationality` varchar(100) NOT NULL,
  `religion` varchar(100) DEFAULT NULL,
  `contact_number` varchar(30) NOT NULL,
  `email_address` varchar(255) DEFAULT NULL,
  `emergency_person` varchar(255) DEFAULT NULL,
  `emergency_person_relation` varchar(100) DEFAULT NULL,
  `emergency_person_number` varchar(20) DEFAULT NULL,
  `form_type` varchar(30) NOT NULL,
  `additional_notes` varchar(1000) DEFAULT NULL,
  `picked_time` varchar(20) NOT NULL,
  `picked_date` date DEFAULT NULL,
  `picked_service` varchar(100) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `queue_form`
--

INSERT INTO `queue_form` (`form_id`, `user_id`, `first_name`, `middle_initial`, `last_name`, `birth_date`, `age`, `gender`, `civil_status`, `patient_type`, `address`, `nationality`, `religion`, `contact_number`, `email_address`, `emergency_person`, `emergency_person_relation`, `emergency_person_number`, `form_type`, `additional_notes`, `picked_time`, `picked_date`, `picked_service`) VALUES
(1, 5, 'Sage Luther', 'B', 'Cui', '2006-10-17', 19, 'Female', 'Single', 'Regular', 'Cubacub Mandaue City', 'Filipino', 'Roman Catholic', '09919106710', 'venzblanz@gmail.com', 'SDASD', 'Mother', 'asdasdasdasd', 'Women\'s Health', 'SAASDADASD', '9:00 AM', '2026-05-18', 'Check-up'),
(2, 5, 'ASDSDSD', 'D', 'ASDASDD', '2007-06-19', 18, 'Female', 'Widowed', 'Pregnant', 'ASDASDASDSD', 'Japanese', 'Jehovah\'s Witness', '09919106710', 'asdasdasdasdasd', 'ASDSDSD', 'Grandfather', '09919106710', 'Specialized Fields', 'ASDASDASDSD', '9:00 AM', '2026-05-18', 'Pediatrics');

-- --------------------------------------------------------

--
-- Table structure for table `queue_line`
--

CREATE TABLE `queue_line` (
  `queue_id` int(11) NOT NULL,
  `form_id` int(11) NOT NULL,
  `user_id` int(11) NOT NULL,
  `department` varchar(100) NOT NULL,
  `queue_number` varchar(20) NOT NULL,
  `status` varchar(30) DEFAULT 'Waiting',
  `created_at` datetime DEFAULT current_timestamp(),
  `staff_assigned` varchar(100) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `queue_line`
--

INSERT INTO `queue_line` (`queue_id`, `form_id`, `user_id`, `department`, `queue_number`, `status`, `created_at`, `staff_assigned`) VALUES
(1, 1, 5, 'Women\'s Health', 'W-01', 'Waiting', '2026-05-17 12:15:33', ''),
(2, 2, 5, 'Specialized Fields', 'S-01', 'Waiting', '2026-05-17 12:18:53', '');

-- --------------------------------------------------------

--
-- Table structure for table `schedules`
--

CREATE TABLE `schedules` (
  `schedule_id` int(11) NOT NULL,
  `service_id` int(11) NOT NULL,
  `day_of_week` tinyint(4) DEFAULT NULL,
  `specific_date` date DEFAULT NULL,
  `time_slot` varchar(20) NOT NULL,
  `doctor_name` varchar(150) DEFAULT NULL COMMENT 'Doctor assigned to this specific slot. NULL = no doctor assigned yet.'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `schedules`
--

INSERT INTO `schedules` (`schedule_id`, `service_id`, `day_of_week`, `specific_date`, `time_slot`, `doctor_name`) VALUES
(1, 1, 1, NULL, '8:00 AM', 'Dr. Maria Santos'),
(2, 1, 1, NULL, '9:00 AM', 'Dr. Maria Santos'),
(3, 1, 1, NULL, '10:00 AM', 'Dr. Maria Santos'),
(4, 1, 1, NULL, '1:00 PM', 'Dr. Maria Santos'),
(5, 1, 1, NULL, '2:00 PM', 'Dr. Maria Santos'),
(6, 1, 2, NULL, '8:00 AM', 'Dr. Maria Santos'),
(7, 1, 2, NULL, '9:00 AM', 'Dr. Maria Santos'),
(8, 1, 2, NULL, '10:00 AM', 'Dr. Maria Santos'),
(9, 1, 2, NULL, '1:00 PM', 'Dr. Maria Santos'),
(10, 1, 2, NULL, '2:00 PM', 'Dr. Maria Santos'),
(11, 1, 3, NULL, '8:00 AM', 'Dr. Maria Santos'),
(12, 1, 3, NULL, '9:00 AM', 'Dr. Maria Santos'),
(13, 1, 3, NULL, '10:00 AM', 'Dr. Maria Santos'),
(14, 1, 3, NULL, '1:00 PM', 'Dr. Maria Santos'),
(15, 1, 3, NULL, '2:00 PM', 'Dr. Maria Santos'),
(16, 1, 4, NULL, '8:00 AM', 'Dr. Ramon Dela Cruz'),
(17, 1, 4, NULL, '9:00 AM', 'Dr. Ramon Dela Cruz'),
(18, 1, 4, NULL, '10:00 AM', 'Dr. Ramon Dela Cruz'),
(19, 1, 4, NULL, '1:00 PM', 'Dr. Ramon Dela Cruz'),
(20, 1, 4, NULL, '2:00 PM', 'Dr. Ramon Dela Cruz'),
(21, 1, 5, NULL, '8:00 AM', 'Dr. Ramon Dela Cruz'),
(22, 1, 5, NULL, '9:00 AM', 'Dr. Ramon Dela Cruz'),
(23, 1, 5, NULL, '10:00 AM', 'Dr. Ramon Dela Cruz'),
(24, 1, 5, NULL, '1:00 PM', 'Dr. Ramon Dela Cruz'),
(25, 1, 5, NULL, '2:00 PM', 'Dr. Ramon Dela Cruz'),
(27, 2, 1, NULL, '11:00 AM', 'Dr. Ramon Dela Cruz'),
(28, 2, 1, NULL, '3:00 PM', 'Dr. Ramon Dela Cruz'),
(29, 2, 3, NULL, '8:00 AM', 'Dr. Ramon Dela Cruz'),
(30, 2, 3, NULL, '11:00 AM', 'Dr. Ramon Dela Cruz'),
(31, 2, 3, NULL, '3:00 PM', 'Dr. Ramon Dela Cruz'),
(32, 2, 5, NULL, '8:00 AM', 'Dr. Ramon Dela Cruz'),
(33, 2, 5, NULL, '11:00 AM', 'Dr. Ramon Dela Cruz'),
(34, 2, 5, NULL, '3:00 PM', 'Dr. Ramon Dela Cruz'),
(35, 3, 2, NULL, '9:00 AM', 'Dr. Ana Reyes'),
(36, 3, 2, NULL, '10:00 AM', 'Dr. Ana Reyes'),
(37, 3, 2, NULL, '2:00 PM', 'Dr. Ana Reyes'),
(38, 3, 4, NULL, '9:00 AM', 'Dr. Ana Reyes'),
(39, 3, 4, NULL, '10:00 AM', 'Dr. Ana Reyes'),
(40, 3, 4, NULL, '2:00 PM', 'Dr. Ana Reyes'),
(41, 4, 1, NULL, '9:00 AM', 'Dr. Jose Lim'),
(42, 4, 1, NULL, '11:00 AM', 'Dr. Jose Lim'),
(43, 4, 1, NULL, '1:00 PM', 'Dr. Jose Lim'),
(44, 4, 3, NULL, '9:00 AM', 'Dr. Jose Lim'),
(45, 4, 3, NULL, '11:00 AM', 'Dr. Jose Lim'),
(46, 4, 3, NULL, '1:00 PM', 'Dr. Jose Lim'),
(47, 4, 5, NULL, '9:00 AM', 'Dr. Jose Lim'),
(48, 4, 5, NULL, '11:00 AM', 'Dr. Jose Lim'),
(49, 4, 5, NULL, '1:00 PM', 'Dr. Jose Lim'),
(50, 5, 2, NULL, '10:00 AM', 'Dr. Carla Mendoza'),
(51, 5, 2, NULL, '2:00 PM', 'Dr. Carla Mendoza'),
(52, 5, 2, NULL, '3:00 PM', 'Dr. Carla Mendoza'),
(53, 5, 4, NULL, '10:00 AM', 'Dr. Carla Mendoza'),
(54, 5, 4, NULL, '2:00 PM', 'Dr. Carla Mendoza'),
(55, 5, 4, NULL, '3:00 PM', 'Dr. Carla Mendoza'),
(56, 6, 1, NULL, '8:00 AM', NULL),
(57, 6, 1, NULL, '1:00 PM', NULL),
(58, 6, 1, NULL, '4:00 PM', NULL),
(59, 6, 2, NULL, '8:00 AM', NULL),
(60, 6, 2, NULL, '1:00 PM', NULL),
(61, 6, 2, NULL, '4:00 PM', NULL),
(62, 6, 3, NULL, '8:00 AM', NULL),
(63, 6, 3, NULL, '1:00 PM', NULL),
(64, 6, 3, NULL, '4:00 PM', NULL),
(65, 6, 4, NULL, '8:00 AM', NULL),
(66, 6, 4, NULL, '1:00 PM', NULL),
(67, 6, 4, NULL, '4:00 PM', NULL),
(68, 6, 5, NULL, '8:00 AM', NULL),
(69, 6, 5, NULL, '1:00 PM', NULL),
(70, 6, 5, NULL, '4:00 PM', NULL);

-- --------------------------------------------------------

--
-- Table structure for table `services`
--

CREATE TABLE `services` (
  `service_id` int(11) NOT NULL,
  `service_name` varchar(100) NOT NULL,
  `service_type` varchar(50) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `services`
--

INSERT INTO `services` (`service_id`, `service_name`, `service_type`) VALUES
(1, 'Check-up', 'General Wellness'),
(2, 'Vaccination', 'General Wellness'),
(3, 'Postnatal Care', 'Women\'s Health'),
(4, 'Pediatrics', 'Specialized Fields'),
(5, 'Dental', 'Specialized Fields'),
(6, 'X-Ray', 'Diagnostics & Laboratory');

-- --------------------------------------------------------

--
-- Table structure for table `users`
--

CREATE TABLE `users` (
  `user_id` int(11) NOT NULL,
  `email` varchar(100) NOT NULL,
  `password` varchar(255) NOT NULL,
  `role` enum('patient','admin') DEFAULT 'patient',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `users`
--

INSERT INTO `users` (`user_id`, `email`, `password`, `role`, `created_at`) VALUES
(1, 'caimorjuner@gmail.com', '123123', 'patient', '2026-05-01 11:16:19'),
(2, 'admin@admin.com', '123456', 'admin', '2026-05-04 03:47:37'),
(4, 'johndoe@gmail.com', '123123', 'patient', '2026-05-08 09:20:31'),
(5, 'venzblanz@gmail.com', '123123', 'patient', '2026-05-16 05:15:35');

--
-- Indexes for dumped tables
--

--
-- Indexes for table `patients`
--
ALTER TABLE `patients`
  ADD PRIMARY KEY (`patient_id`),
  ADD KEY `user_id` (`user_id`);

--
-- Indexes for table `queue_form`
--
ALTER TABLE `queue_form`
  ADD PRIMARY KEY (`form_id`),
  ADD KEY `fk_user_id` (`user_id`);

--
-- Indexes for table `queue_line`
--
ALTER TABLE `queue_line`
  ADD PRIMARY KEY (`queue_id`),
  ADD KEY `fk_form_id` (`form_id`),
  ADD KEY `fk1_user_id` (`user_id`);

--
-- Indexes for table `schedules`
--
ALTER TABLE `schedules`
  ADD PRIMARY KEY (`schedule_id`),
  ADD KEY `service_id` (`service_id`);

--
-- Indexes for table `services`
--
ALTER TABLE `services`
  ADD PRIMARY KEY (`service_id`);

--
-- Indexes for table `users`
--
ALTER TABLE `users`
  ADD PRIMARY KEY (`user_id`),
  ADD UNIQUE KEY `email` (`email`);

--
-- AUTO_INCREMENT for dumped tables
--

--
-- AUTO_INCREMENT for table `patients`
--
ALTER TABLE `patients`
  MODIFY `patient_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;

--
-- AUTO_INCREMENT for table `queue_form`
--
ALTER TABLE `queue_form`
  MODIFY `form_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;

--
-- AUTO_INCREMENT for table `queue_line`
--
ALTER TABLE `queue_line`
  MODIFY `queue_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;

--
-- AUTO_INCREMENT for table `schedules`
--
ALTER TABLE `schedules`
  MODIFY `schedule_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=72;

--
-- AUTO_INCREMENT for table `services`
--
ALTER TABLE `services`
  MODIFY `service_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;

--
-- AUTO_INCREMENT for table `users`
--
ALTER TABLE `users`
  MODIFY `user_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6;

--
-- Constraints for dumped tables
--

--
-- Constraints for table `patients`
--
ALTER TABLE `patients`
  ADD CONSTRAINT `patients_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`);

--
-- Constraints for table `queue_form`
--
ALTER TABLE `queue_form`
  ADD CONSTRAINT `fk_user_id` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`);

--
-- Constraints for table `queue_line`
--
ALTER TABLE `queue_line`
  ADD CONSTRAINT `fk1_user_id` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`),
  ADD CONSTRAINT `fk_form_id` FOREIGN KEY (`form_id`) REFERENCES `queue_form` (`form_id`);

--
-- Constraints for table `schedules`
--
ALTER TABLE `schedules`
  ADD CONSTRAINT `schedules_ibfk_1` FOREIGN KEY (`service_id`) REFERENCES `services` (`service_id`);
COMMIT;

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
