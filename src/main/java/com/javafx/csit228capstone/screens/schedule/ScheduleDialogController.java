...
public class ScheduleDialogController implements Initializable {
    ...
    private final Service[] selected = { null };

    @Override
    public void initialize(URL url, ResourceBundle rb) { }

    public void init(String timeSlot, String dayLabel, LocalDate selectedDate, Consumer<Service> onServiceSelected) {
        ...
        dialogSearchField.textProperty().addListener((obs, o, n) -> filterRows(n));
    }
    ...
    private void onAdd() {
        ...
        Service withRecurring = Service.createNewWithId(selected[0].getServiceId(), selected[0].getName(), selected[0].getServiceType(), recurringCheck.isSelected());
    ...
    private void onOtherServiceClicked() {
        try {
            ...
            ctrl.init(selectedDate, getDayName(dialogSubtitle.getText().split("·")[0].trim()), recurringCheck.isSelected(), timeSlot, service -> {
            ...
            dialogAddBtn.getScene().getRoot().setEffect(dim);
    ...
    private String getDayName(String dayLabel) { return dayLabel.split(",")[0].trim(); }
}