...
public class NotificationPanelController {
    ...
    private Timeline queueAlertTimeline;
    ...
    public NotificationPanelController(StackPane rootStack) {
        ...
        startQueueAlertChecker();
        refresh();
    }

    private void startQueueAlertChecker() {

        queueAlertTimeline = new Timeline(
                new KeyFrame(Duration.seconds(10), e -> checkAlmostTurn())
        );

        queueAlertTimeline.setCycleCount(Timeline.INDEFINITE);
        queueAlertTimeline.play();

        checkAlmostTurn();
    }

    private void checkAlmostTurn() {
            ...
            protected Void call() {
                ...
                String sql = """
                    SELECT q.queue_number,
                           ...
                           (
                               SELECT COUNT(*)
                               FROM queue_line q2
                               WHERE q2.department = q.department
                ...
                try (Connection c = DatabaseConfig.getConnection();
                    ...
                    if (rs.next()) {
                        ...
                        int ahead = rs.getInt("people_ahead");
                        ...
                        if (ahead <= 5) {
                            ...
                            if (!hasExistingAlert(queueNumber)) {
                                ...
                                if (ahead == 0) {
                                    ...
                                    message = "You're next in line for " + dept +
                                            ". Please prepare for your turn. Queue #" + queueNumber;
                                } else {
                                    title = "You're Almost Next";
                                    message = "Only " + ahead + " patient(s) ahead of you in " + dept +
                                            ". Queue #" + queueNumber;
                                }
                                ...
                                System.out.println("[Notif] Inserted almost-turn alert");
                            }
                        }
                    }

                } catch (Exception e) {
                    System.err.println("[Notif Alert] " + e.getMessage());
                }
    ...
    private boolean hasExistingAlert(String queueNumber) {

        String sql = """
            SELECT COUNT(*)
            FROM notifications
            WHERE user_id = ?
            AND type = 'ALMOST_TURN'
            AND message LIKE ?
        ...
        try (Connection c = DatabaseConfig.getConnection();
            ...
            System.err.println("[Notif Duplicate Check] " + e.getMessage());
        }
